...
import { ApiError, apiGet, apiPatch, apiPost, apiPostForm } from './lib/api';
...
function HomePage({ ctx }: { ctx: AppContext }) {
  ...
  async function initSchema() {
    const data = await apiPost<Record<string, unknown>>(ctx, '/api/graph/init-schema');
    setSchemaResult(data);
  }

  return <>
    <PageTitle title="Научный клубок" subtitle="Статус сервисов и инициализация графовой схемы." />
    ...
    <Card>
      ...
      <Button disabled={loading || ctx.role !== 'admin'} onClick={() => run(initSchema)}>{loading ? 'Initializing…' : 'Init Neo4j schema'}</Button>
      {ctx.role !== 'admin' ? <small>Нужна роль admin.</small> : null}
      {schemaResult ? <Notice tone={schemaResult.status === 'ok' ? 'success' : 'warning'}>
        Schema initialized: {String(schemaResult.constraints_and_indexes ?? '—')} constraints/indexes; fulltext: {String(schemaResult.fulltext_indexes ?? '—')}.
      </Notice> : null}
    </Card>
...
function ImportPage({ ctx }: { ctx: AppContext }) {
  ...
  return <>
    <PageTitle title="Import" subtitle="Загрузка документов, извлечение текста и последующая обработка." />
    <RoleGate ctx={ctx} allowed={INTERNAL_ROLES} action="Импорт документов">
      {error ? <Notice tone="danger">{error}</Notice> : null}
      <Card>
        <h2>Upload file</h2>
        <Notice tone="info">Поддерживаются PDF/DOCX/XLSX/TXT/MD/CSV до 50 MB.</Notice>
        <label>Файл</label>
...
function SearchPage({ ctx, lastSearch, setLastSearch, setPage }: { ctx: AppContext; lastSearch: SearchResponse | null; setLastSearch: (s: SearchResponse) => void; setPage: (p: PageKey) => void }) {
  ...
  return <>
    ...
    {ctx.role === 'external_partner' ? <Notice tone="warning">Вы в роли external_partner: видны только public-данные.</Notice> : null}
    {error ? <Notice tone="danger">{error}</Notice> : null}
    <div className="two-col wide-left">
      <Card>
        ...
        <div className="search-submit"><Button disabled={loading} onClick={() => run(search)}>Найти</Button></div>
      </Card>
...
export default function App() {
  ...
  const [page, setPage] = useState<PageKey>(() => {
    const stored = sessionStorage.getItem('sk_page');
    const validPages: PageKey[] = ['home', 'import', 'search', 'graph', 'facts', 'dashboard', 'export', 'audit'];
    return stored && validPages.includes(stored as PageKey) ? stored as PageKey : 'home';
  });
