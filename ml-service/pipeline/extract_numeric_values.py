...
def infer_parameter_near_match(
    text: str,
    start: int,
    end: int,
    unit_normalized: str,
    ...
    """Infer parameter using distance from semantic cue to this exact numeric span.

    The old implementation scored a broad context window uniformly, which could map
    `95 % recovery` to `temperature` merely because a temperature was mentioned in
    the same sentence. Here closer cues win.
    """
    left = max(0, start - window)
    right = min(len(text), end + window)
    ctx = text[left:right].lower()
    numeric_center = ((start + end) / 2.0) - left
    ...
    for parameter, patterns in PARAMETER_RULES:
        score = 0.0
        for pattern in patterns:
            for match in re.finditer(pattern, ctx, flags=re.IGNORECASE):
                cue_center = (match.start() + match.end()) / 2.0
                distance = abs(cue_center - numeric_center)
                # Strongly reward nearby cues, but still allow sentence-level context.
                score += 1.0 / (1.0 + distance / 24.0)
        if score > best_score:
            best_score = score
            best_parameter = parameter

    # Unit priors are deterministic and should dominate weak distant cues.
    unit_defaults = {
        "degC": ("temperature", 1.25),
        "m/s": ("flow_velocity", 1.0),
        "m3/h": ("flow_rate", 0.9),
        "m3/day": ("flow_rate", 0.9),
        "t/day": ("productivity", 0.9),
        "t/year": ("productivity", 0.9),
        "MPa": ("pressure", 1.1),
        "kPa": ("pressure", 1.1),
        "Pa": ("pressure", 1.1),
        "atm": ("pressure", 1.1),
        "pH": ("ph", 1.25),
        "RUB/m3": ("economic_indicator", 1.0),
        "million_RUB": ("economic_indicator", 1.0),
        "thousand_RUB": ("economic_indicator", 1.0),
        "g/t": ("content", 0.75),
        "kg/t": ("content", 0.75),
    }
    default = unit_defaults.get(unit_normalized)
    if default and default[1] > best_score:
        best_parameter, best_score = default

    if best_score == 0:
        if unit_normalized in {"mg/L", "g/L", "ug/L", "ppm"}:
            return "concentration", 0.6
        if unit_normalized == "%":
            return "share_percent", 0.5
        if unit_normalized in {"mm", "cm", "m", "km"}:
            return "dimension", 0.5

    return best_parameter, min(best_score, 1.0)
...
def find_numeric_values(
    ...
    def add_result(
        ...
        parameter, parameter_score = infer_parameter_near_match(text, start, end, unit_normalized)
